<?php defined('BASEPATH') OR exit('No direct script access allowed'); $page_title = 'My Scam Reports'; ?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    :root{
      --ink:#1e3a8a; --brand:#2563eb; --muted:#64748b; --line:#d9dee7;
      --chip:#eef2ff; --bg:#f6f8fc; --bg2:#eef2f7; --radius:14px; --shadow:0 8px 22px rgba(2,6,23,.10)
    }
    body{background:linear-gradient(180deg,var(--bg),var(--bg2) 70%,#e9edf3)}
    .app{max-width:1060px;margin:0 auto;padding:0 14px}
    .page-head{display:flex;align-items:center;gap:12px;margin:6px 0 16px}
    .page-head .icon{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;background:var(--chip)}
    .page-head .icon i{font-size:20px;color:var(--brand)}
    .page-head .title{margin:0;font:700 24px/1.2 Poppins;color:var(--ink)}
    ...
    .card-head h6{margin:0;font:800 13px/1 Poppins;color:var(--ink)}
    .toolbar{margin-left:auto;display:flex;gap:8px}
    .btn-brand{background:#f5f8ff;border:1px solid var(--brand);color:var(--ink);border-radius:12px;font-weight:700;padding:.45rem .85rem}
    .btn-brand:hover{background:#e9f0ff}

    .table-wrap{padding:12px 16px}
    .table-r{margin-bottom:0}
    .table-r thead th{background:#f9fbff;border-top:0;position:sticky;top:0;z-index:1}
    .table-r th,.table-r td{vertical-align:middle}
    .table-r tbody tr{cursor:pointer}
    .table-r tbody tr:hover{background:#fbfdff}
    .badge-chip{border-radius:999px;padding:.35rem .6rem;font-weight:600;font-size:11.5px;letter-spacing:.25px}
    .badge-type{background:#ffe5e8;color:#9a0820;border:1px solid #ffc4cb}
    .muted{color:var(--muted)}
    .empty-row{height:120px}
    ...
    @media (max-width: 768px) {
      ...
      .table-r { border-collapse: separate; border-spacing: 0 12px; }
      .table-r tbody, .table-r tr, .table-r td { display: block; width: 100%; }
      ...
      .table-r tbody tr:hover { background: #fff; }
      ...
      .table-r td:last-child { border-bottom: 0; padding-bottom: 2px; }
      .table-r td::before {
      ...
      .table-r .text-truncate { max-width: 100% !important; }
      .status-pill { justify-self: start; }
      .text-muted { color: #6b7280 !important; }
      .toolbar { width: 100%; }
      .toolbar .btn-brand { width: 100%; text-align: center; }
      .truncate-2 { max-width: 100%; }
    }
    ...
    .icon-btn i{ font-size:18px; color:#0f172a; opacity:.82 }
    .icon-btn--del i{ color:#9a0820 }
    /* Title + inline details */
.title-line{display:flex; align-items:center; gap:10px; min-width:0;}
.title-txt{font-weight:600; color:#1e3a8a; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:320px;}
.title-sep{opacity:.35}
.details-inline{color:#64748b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:400px;}
@media (max-width:768px){
  .title-txt,.details-inline{max-width:100%}
}
...
<script>
  (function () {
    ...
    obs.observe(table.tBodies[0], { childList: true, subtree: true });
  })();
</script>
