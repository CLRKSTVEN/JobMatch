  ...
  <head>
    ...
    <style>
      .select2-container .select2-selection--single{
        height:42px;border:1px solid #d1d5db;border-radius:.5rem;
      }
      .select2-container--default .select2-selection--single .select2-selection__rendered{
        line-height:42px;padding-left:.75rem;
      }
      .select2-container--default .select2-selection--single .select2-selection__arrow{
        height:42px;right:.5rem;
      }
      ...
      .status-badge{
        display:inline-flex;align-items:center;gap:.5rem;color:#fff;background:rgba(255,255,255,.12);
        border:1px solid rgba(255,255,255,.25);border-radius:9999px;padding:.4rem .75rem;line-height:1;
        box-shadow:0 1px 0 rgba(255,255,255,.12) inset, 0 8px 24px rgba(0,0,0,.12);backdrop-filter:saturate(140%) blur(4px);
      }
      @media (max-width:768px){ .status-badge{ align-self:flex-start; } }

      #toaster{ position:fixed; inset:0; display:flex; justify-content:center; align-items:center; z-index:9999; pointer-events:none; padding:16px; }
      .toast{ pointer-events:auto; max-width:min(92vw,520px); text-align:center; background:#111827; color:#fff; border-radius:12px; padding:.75rem 1rem; box-shadow:0 10px 30px rgba(0,0,0,.25); font-size:.95rem; font-weight:600; animation:toast-in .18s ease-out; }
      ...
      .wk-guide-highlight{
        outline:3px solid #2563eb!important; box-shadow:0 0 0 6px rgba(37,99,235,.12), 0 8px 30px rgba(37,99,235,.22)!important;
        border-radius:.6rem!important; transition:box-shadow .18s ease; position:relative;
      }
      .wk-guide-highlight::after{
        content:""; position:absolute; inset:-6px; border:2px dashed rgba(37,99,235,.5); border-radius:.8rem; animation:wkPulse 1.4s ease-in-out infinite; pointer-events:none;
      }
      @keyframes wkPulse{ 0%{opacity:.5; transform:scale(.98)} 50%{opacity:1; transform:scale(1)} 100%{opacity:.5; transform:scale(.98)} }
    </style>
<style>
  .doc-card { border: 1px solid #e5e7eb; border-radius: 14px; background: #fff; }
  .doc-toolbar { display:flex; align-items:center; justify-content:space-between; gap:.75rem; }
  .doc-toolbar h2 { margin:0; }

  .doc-empty {
    padding: 40px 16px; text-align:center; color:#6b7280;
    background: linear-gradient(180deg,#fafafa, #fff);
    border-radius: 12px;
  }
  .doc-empty .art {
    width: 80px; height:80px; margin: 0 auto 12px; border-radius: 16px;
  ...
  #documentsTable { font-size: .92rem; }
  #documentsTable thead th {
    background:#f8fafc!important; color:#334155; font-weight:600; border-bottom:1px solid #e5e7eb!important;
  }
  #documentsTable tbody tr { transition: background .12s ease; }
  #documentsTable tbody tr:hover { background: #f9fafb; }
  #documentsTable td, #documentsTable th { padding: 12px 14px!important; }
  .chip { display:inline-flex; align-items:center; gap:.5rem; padding:.25rem .6rem; border-radius:999px; font-size:.75rem; line-height:1; border:1px solid; }
  .chip--type { color:#1e3a8a; background:#eff6ff; border-color:#bfdbfe; }
  .chip--skill { color:#065f46; background:#ecfdf5; border-color:#a7f3d0; }
  .badge { display:inline-flex; align-items:center; gap:.4rem; padding:.25rem .5rem; border-radius:8px; font-size:.72rem; font-weight:600; }
  ...
  .file-pill { display:inline-flex; align-items:center; gap:.5rem; padding:.3rem .6rem; border-radius:10px; background:#f1f5f9; color:#0f172a; }
  .file-pill svg { width:14px; height:14px; }
  .btn-ghost { background:#f3f4f6; color:#374151; border-radius:10px; padding:.45rem .65rem; font-size:.78rem; }
  .btn-ghost:hover { background:#e5e7eb; }
  .btn-danger { background:#dc2626; color:#fff; border-radius:10px; padding:.45rem .65rem; font-size:.78rem; }
  .btn-danger:hover { background:#b91c1c; }
  #docModalRoot .form-label { color:#374151; }
  #docModalRoot input[type="file"] { border: 1px dashed #cbd5e1; border-radius: 12px; padding: 10px; background:#f8fafc; }
</style>
<style>
  @media (max-width: 768px) {
    .main-container { border: 0; border-radius: 0; box-shadow: none; }

    .header-gradient { border-radius: 0; padding: 16px 16px 18px; }
    ...
    .m-gap-compact { gap: 14px !important; }
    ...
    #experiencesTable { font-size:.92rem; }
  #experiencesTable thead th{
    background:#f8fafc!important;color:#334155;font-weight:600;
    border-bottom:1px solid #e5e7eb!important;
  }
  #experiencesTable td, #experiencesTable th{ padding:12px 14px!important; }
  #experiencesTable tbody tr{ transition:background .12s ease; }
  #experiencesTable tbody tr:hover{ background:#f9fafb; }

  .chip--employer{ color:#1f2937;background:#f3f4f6;border-color:#e5e7eb; } /* grey pill like â€œTypeâ€ */
  .period-badge{ display:inline-flex;align-items:center;gap:.4rem;
    padding:.25rem .5rem;border-radius:8px;font-size:.72rem;font-weight:600;
    color:#1e3a8a;background:#eff6ff;
  }
  .desc-cell{
    max-width:520px; display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical;
    overflow:hidden; color:#334155;
  }
  .desc-more{ display:inline-block;margin-left:.5rem;font-size:.75rem;color:#2563eb; }
</style>
  ...
  <?php
    ...
    if ($avatarField !== '' && preg_match('#^https?://#i', $avatarField)) {
        ...
        if (!is_file(FCPATH.$rel)) {
            if (is_file(FCPATH.$DEFAULT_AVATAR_REL)) { $rel = $DEFAULT_AVATAR_REL; }
            else { $rel = null; }
        }
        if ($rel) { $avatarLocalRel = $rel; $avatarUrl = base_url($rel); }
        else { $avatarUrl = 'https://api.dicebear.com/9.x/initials/svg?seed='.urlencode($seed); }
    }
    ...
    if (!empty($p->tesda_certs)) {
      ...
      if (is_array($rows)) {
        foreach ($rows as $row) {
          ...
          if ($q !== '' && isset($mapByText[$q])) { $currentNCs[] = $mapByText[$q]; }
        }
      }
    }
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-3 mt-3">
                    <?php foreach ($days as $d):
                      $id  = 'day_'.$d; $chk = in_array($d, $picked) ? 'checked' : ''; ?>
                ...
                <div>
                  ...
                  <div class="skills-search-wrap mt-1">
                    <div class="flex items-center border border-gray-200 rounded-xl bg-white px-3 py-2">
                      <svg class="w-4 h-4 text-gray-400 mr-2" viewBox="0 0 24 24" fill="currentColor"><path d="M21 21l-4.35-4.35M10 18a8 8 0 110-16 8 8 0 010 16z"/></svg>
...
<div class="doc-card p-3">
  ...
  <div id="wkGuide" class="fixed bottom-4 right-4 z-[9998] hidden">
    <button id="wkGuideBeacon" class="shadow-lg rounded-full px-4 py-2 bg-blue-600 text-white text-sm font-semibold flex items-center gap-2">
      <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z"/></svg>
      Guided steps
    </button>
...
<div class="fixed inset-0 z-[9999] hidden" id="expModalRoot" aria-hidden="true">
  ...
  ?>
  <script id="skillOptionsJson" type="application/json"><?= $skillOptionsJson ?></script>
  <script id="ncOptionsJson"  type="application/json"><?= $ncOptionsJson ?></script>
  <script id="currentNCsJson" type="application/json"><?= $currentNCsJson ?></script>
...
<script>
  ...
  document.getElementById('docModalRoot').classList.remove('hidden');
    ...
    function closeDocModal(){ document.getElementById('docModalRoot').classList.add('hidden'); }
  ...
  function ensureDocTypes(){
    ...
    return fetch('<?= site_url('profile/doc_types_json') ?>', {credentials:'same-origin'})
      .then(r => r.json())
      .then(list => {
...
function isNonTesda(text, code){ return !isTesdaType(text, code); }
...
,
    ...
    },
 {
  ...
  render: (d, _type, row) => {
    ...
    { data: 'expiry_date', title: 'Expiry', render: d => {
       ...
       if (!exp.isValid()) return htmlEscape(d);
        ...
        let cls = 'badge--ok', txt = exp.format('YYYY-MM-DD');
        if (diff < 0)        { cls = 'badge--danger'; txt = `Expired ${exp.fromNow()}`; }
        else if (diff <= 30) { cls = 'badge--warn';   txt = `In ${diff} day${diff===1?'':'s'} (${exp.format('YYYY-MM-DD')})`; }
...
{ data: 'file_path', title: 'File', render: d => {
    if (!d) return '<span class="text-slate-400">—</span>';
...
$('#btnAddDoc').on('click', function(){ openDocModal(false, null); });
    $('#documentsTable').on('click', '[data-doc-edit]', async function(){
      ...
      await ensureSkills(); openDocModal(true, row);
    });

    $('#documentsTable').on('click', '[data-doc-del]', async function(){
      const id = this.getAttribute('data-doc-del'); if (!id) return;
    ...
    document.getElementById('docModalRoot').addEventListener('click', (e)=>{ if (e.target.id === 'docModalRoot') closeDocModal(); });

    document.getElementById('docForm').addEventListener('submit', async (e)=>{
      e.preventDefault();
      const fd = new FormData(e.currentTarget);
      try {
        const res  = await fetch('<?= site_url('profile/save_document') ?>', { method:'POST', credentials:'same-origin', body:fd, headers:{ 'X-Requested-With':'XMLHttpRequest' } });
        const ct   = (res.headers.get('Content-Type')||'').toLowerCase();
        const json = ct.includes('application/json') ? await res.json() : { ok:false, error:'Non-JSON response' };
        if (!res.ok || !json.ok) throw new Error(json.error || 'Save failed');
        closeDocModal(); table.ajax.reload(null,false);
...
(function ExperiencesTableAndModal(){
  ...
  function toggleExpVisibility(n){ (n>0?expWrap:expEmpty).classList.remove('hidden'); (n>0?expEmpty:expWrap).classList.add('hidden'); }
  ...
  const expTable = $('#experiencesTable').DataTable({
    ...
    ajax: {
      ...
      error: () => { toggleExpVisibility(0); window.toast && toast('Failed to load experiences', false); }
    },
    columns: [
      { data: 'role', title: 'Role / Title', responsivePriority: 1,
        render: d => d ? `<div class="font-medium text-slate-800">${htmlEscape(d)}</div>` : '(Untitled)' },
      { data: 'employer', title: 'Employer / Project', responsivePriority: 2,
        render: d => d ? `<span class="chip chip--employer">${htmlEscape(d)}</span>` : '<span class="text-slate-400">—</span>' },
      { data: null, title: 'Period', responsivePriority: 3, render: renderPeriod },
      { data: 'desc', title: 'Description', responsivePriority: 4, render: renderDesc },
    ...
    order: [[2, 'desc']],    // newest period first
    columnDefs: [
      { targets: 4, width: 120 } // Action
    ]
  });
  ...
  function openExpModal(edit=false, row=null){
    ...
    if (edit && row){
      ...
      presEl.checked = isPresent; toEl.disabled = isPresent;
      toEl.value     = (!isPresent && row.to && /^\d{4}-\d{2}$/.test(row.to)) ? row.to : (!isPresent && row.to && /^\d{4}$/.test(row.to)) ? `${row.to}-01` : '';
      descEl.value   = row.desc || '';
    } else { presEl.checked=false; toEl.disabled=false; }
    root.classList.remove('hidden');
  }
  function closeExpModal(){ root.classList.add('hidden'); }
  ...
  document.getElementById('experiencesTable').addEventListener('click', (e)=>{
    ...
    if (editBtn){
      ...
      const id = delBtn.getAttribute('data-exp-del'); if (!id) return;
      ...
      (async ()=>{
        ...
        }catch(err){ console.error(err); alert('Delete failed: '+err.message); window.toast && toast('Delete failed', false); }
      })();
    }
  });

  presEl.addEventListener('change', ()=>{ toEl.disabled = presEl.checked; if (presEl.checked) toEl.value=''; });

  form.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const fd = new FormData(form);
    if (presEl.checked){ fd.set('to_present','1'); fd.set('to',''); } else { fd.delete('to_present'); }
    try{
      const res  = await fetch('<?= site_url('profile/save_experience') ?>', { method:'POST', body:fd, credentials:'same-origin', headers:{ 'X-Requested-With':'XMLHttpRequest' }});
      const ct   = (res.headers.get('Content-Type')||'').toLowerCase();
      const json = ct.includes('application/json') ? await res.json() : { ok:false, error:'Non-JSON response' };
      if (!res.ok || !json.ok) throw new Error(json.error||'Save failed');
      closeExpModal(); expTable.ajax.reload(null,false);
      window.toast && toast(json.mode==='updated' ? 'Experience updated' : 'Experience added', true);
    }catch(err){ console.error(err); alert('Save failed: '+err.message); window.toast && toast('Save failed', false); }
  });

  closeB.addEventListener('click', closeExpModal);
  cancel.addEventListener('click', closeExpModal);
  document.getElementById('expModalRoot').addEventListener('click', e=>{ if (e.target.id==='expModalRoot') closeExpModal(); });
})();
</script>
  ...
  (function(){
    ...
    window.jQuery(function($){
      const $prov = $('#province');
      if ($prov.length) {
        $prov.select2({ placeholder:'- Select Province -', allowClear:true, width:'100%', dropdownParent:$prov.closest('.input-group') });
      }
    });
    ...
    window.toast = function(msg, ok=true){
      ...
      box.appendChild(t);
      setTimeout(()=>{ t.style.transition='opacity .18s ease, transform .18s ease'; t.style.opacity='0'; t.style.transform='translateY(-8px) scale(.98)'; }, 2200);
    ...
    (function initTabs(){
      ...
      function showTab(key){
        ...
        btn.classList.add('active','bg-gray-900','text-white'); panel.classList.remove('hidden');
    ...
    (function initBio(){
      const bio = $('#bio'), cnt = $('#bioCount');
      function upd(){ if (bio && cnt) cnt.textContent = (bio.value.length || 0) + ' / 600'; }
      bio && bio.addEventListener('input', upd); upd();
    })();
    ...
    (function initAvatar(){
      ...
      if (avatarForm) {
        avatarForm.addEventListener('submit', async (e) => {
          ...
          try {
            const res = await fetch(avatarForm.action, { method:'POST', body:fd, credentials:'same-origin' });
            const ct  = (res.headers.get('Content-Type') || '').toLowerCase();
            let data = null; if (ct.includes('application/json')) { data = await res.json(); }
            if (!res.ok) throw new Error('HTTP '+res.status);
            if (data && data.ok && data.avatar_url && avatarImg) { avatarImg.src = data.avatar_url + '?t=' + Date.now(); }
            ...
            console.error(err); toast('Upload failed. Please try again.', false);
          }
        });
      }
    })();
    ...
    (function initAvailability(){
      ...
      const pick = id => $('#'+id);
      pick('pickWeekdays')?.addEventListener('click', () => { days.forEach((d,i)=> $('#day_'+d).checked = i < 5); });
      pick('pickAll')?.addEventListener('click', () => { days.forEach(d=> $('#day_'+d).checked = true); });
      pick('clearDays')?.addEventListener('click', () => { days.forEach(d=> $('#day_'+d).checked = false); });
    })();
    (async function initAddressCascade(){
      const provinceSel = $('#province'), citySel = $('#city'), brgySel = $('#brgy');
      ...
      function fillOptions(selectEl, items, placeholder){
        ...
        if ($sel && $sel.data('select2')) {
          $sel.empty(); $sel.append(new Option(ph, '', true, false));
          items.forEach(v => $sel.append(new Option(v, v, false, false)));
          $sel.trigger('change.select2');
        } else {
          selectEl.innerHTML = ''; const opt0 = document.createElement('option');
          opt0.value = ''; opt0.textContent = ph; selectEl.appendChild(opt0);
          items.forEach(v => { const opt = document.createElement('option'); opt.value = v; opt.textContent = v; selectEl.appendChild(opt); });
        }
      }
      ...
      async function api(params){
        ...
        const text = await res.text(); if (!res.ok) throw new Error('HTTP '+res.status+' '+text.slice(0,120));
        let json; try { json = JSON.parse(text); } catch { throw new Error('API did not return JSON'); }
      ...
      async function loadProvinces(){
        ...
        try {
          let items = await api({scope:'province'}); if (lockProvince) items = [LIMIT_PROVINCE];
          fillOptions(provinceSel, items, '- Select Province -');
        } catch (e) {
          console.error(e); fillOptions(provinceSel, [], 'Unavailable — refresh to retry'); toast('Address service error: '+ e.message, false); return;
        }
        if (preProvince && Array.from(provinceSel.options).some(o => o.value === preProvince)) {
          const $prov = window.jQuery && window.jQuery('#province');
          if ($prov && $prov.length && $prov.data('select2')) { $prov.val(preProvince).trigger('change.select2'); }
          else { provinceSel.value = preProvince; }
      ...
      async function onProvinceChange(isInit=false){
        ...
        citySel.disabled = true; brgySel.disabled = true;
        fillOptions(citySel, [], '- Select City -'); fillOptions(brgySel, [], '- Select Barangay -');
        if (!province) return;
        try {
          const items = await api({scope:'city', province}); fillOptions(citySel, items, '- Select City -');
        } catch (e) {
          console.error(e); fillOptions(citySel, [], 'Unavailable — refresh to retry'); toast('City load error: '+ e.message, false); return;
        } finally { citySel.disabled = false; }
        if (isInit && preCity && Array.from(citySel.options).some(o => o.value === preCity)) { citySel.value = preCity; await onCityChange(true); }
      }

      async function onCityChange(isInit=false){
        const province = lockProvince ? LIMIT_PROVINCE : provinceSel.value;
        const city = citySel.value;
        brgySel.disabled = true; fillOptions(brgySel, [], '- Select Barangay -');
        if (!province || !city) return;
        try {
          const items = await api({scope:'brgy', province, city}); fillOptions(brgySel, items, '- Select Barangay -');
        } catch (e) {
          console.error(e); fillOptions(brgySel, [], 'Unavailable — refresh to retry'); toast('Barangay load error: '+ e.message, false); return;
        } finally { brgySel.disabled = false; }
        if (isInit && preBrgy && Array.from(brgySel.options).some(o => o.value === preBrgy)) { brgySel.value = preBrgy; }
      }
  ...
  (function WorkerGuidePersistent(){
    ...
    const guide = $('#wkGuide'); if (!guide) return;
    ...
    function openTab(key){ const btn = document.querySelector('.tab-button[data-tab="'+key+'"]'); if (btn) btn.click(); }

    const steps = [
      { key:'photo', title:'Add a clear profile photo',
        msg:'Clients respond more to profiles with a clear headshot. Proceed to upload.',
        go: ()=>{ openTab('info'); focusTarget($('#avatarForm .file-upload-area') || $('#avatarPreview')); }},

      { key:'info', title:'Fill your basic information',
        msg:'Enter your name, headline and a short professional summary.',
        go: ()=>{ openTab('info'); focusTarget($('#first_name') || $('#headline') || $('#bio'), true); }},

      { key:'skills', title:'Add your skills',
        msg:'Add at least 3 skills that match your work.',
        go: ()=>{ openTab('skills'); focusTarget($('#skillsSearch') || $('#skillsSelected'), true); }},

      { key:'docs', title:'Add your documents',
        msg:'Click “Add Document”, choose a type. If you pick TESDA/NC/Qualification, you can select a certificate and set an expiry.',
        go: ()=>{ openTab('docs'); focusTarget(document.querySelector('#btnAddDoc'), true); }},

      { key:'availability', title:'Set your availability',
        msg:'Pick the days you are available to work.',
        go: ()=>{ openTab('availability'); focusTarget($('#pickWeekdays') || $('[name="availability_days[]"]'), true); }},

      { key:'save', title:'Save your profile',
        msg:'All done? Save to update your public profile.',
        go: ()=>{ focusTarget(document.querySelector('button[type="submit"]'), true); }}
    ];
    ...
    function showBeacon(){ beacon.classList.remove('hidden'); card.classList.add('hidden'); }
    function showCard(){ beacon.classList.add('hidden'); card.classList.remove('hidden'); }
    function minimize(){ showBeacon(); removeHL(); }
    ...
    function paint(){
      ...
      if (done) {
        ...
        for (let i=0;i<steps.length;i++){ const d=document.createElement('span'); d.className='w-2 h-2 rounded-full bg-blue-600'; dotsEl.appendChild(d); }
        goBtn.textContent = 'Restart'; skipBtn.textContent = 'Close'; return;
      }
      const s = steps[idx];
      titleEl.textContent = `Step ${idx+1} of ${steps.length}: ${s.title}`;
      msgEl.textContent   = s.msg;
      for (let i=0;i<steps.length;i++){
        const d=document.createElement('span'); d.className='w-2 h-2 rounded-full ' + (i<=idx?'bg-blue-600':'bg-gray-300'); dotsEl.appendChild(d);
      }
      goBtn.textContent = 'Proceed'; skipBtn.textContent = 'Skip';
    }

    let lastHL=null;
    function focusTarget(t, focusInput){
      removeHL(); if (!t) return;
      t.classList.add('wk-guide-highlight'); lastHL=t;
      t.scrollIntoView({behavior:'smooth', block:'center'});
      if (focusInput && typeof t.focus==='function') setTimeout(()=>t.focus({preventScroll:true}),450);
    }
    function removeHL(){ if (lastHL){ lastHL.classList.remove('wk-guide-highlight'); lastHL=null; } }

    beacon.addEventListener('click', ()=>{ showCard(); paint(); });
    goBtn.addEventListener('click', ()=>{ if (idx >= steps.length) { idx = 0; localStorage.setItem(LS_STEP, '0'); paint(); return; } const s = steps[idx]; s.go && s.go(); if (idx < steps.length) { idx++; localStorage.setItem(LS_STEP, String(idx)); } paint(); });
    skipBtn.addEventListener('click', ()=>{ if (idx < steps.length) { idx++; localStorage.setItem(LS_STEP, String(idx)); paint(); } else { minimize(); } });
    close.addEventListener('click', minimize);

    guide.classList.remove('hidden'); showBeacon();
  })();
  </script>
