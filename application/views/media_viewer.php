...
<head>
  ...
  <title><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?> • Trabawho</title>
...
?>
  <style>
    :root{
      --bg:#0b1220; --panel:#0f172a; --ink:#e5e7eb; --muted:#94a3b8; --line:#1f2937;
      --btn:#1f2937; --btn-hover:#111827;
    }
    *{box-sizing:border-box}
    ...
    header{display:flex;align-items:center;gap:12px;padding:10px 14px;background:var(--panel);border-bottom:1px solid var(--line)}
    header img.logo{width:22px;height:22px;border-radius:4px}
    header .ttl{font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    header .spacer{flex:1}
    .toolbar{display:flex;gap:8px;flex-wrap:wrap}
    .btn{
      display:inline-flex;align-items:center;gap:6px;
      padding:.45rem .7rem;background:transparent;border:1px solid var(--btn);
      border-radius:8px;color:var(--ink);text-decoration:none;cursor:pointer;user-select:none
    }
    .btn:hover{background:var(--btn-hover)}
    .btn[disabled]{opacity:.5;cursor:not-allowed}

    main{position:relative}
    .stage{position:absolute; inset:0; overflow:hidden; background:radial-gradient(ellipse at center, #0d1326 0%, #0b1220 60%)}

    .canvas{position:absolute; top:50%; left:50%; will-change:transform}
    .canvas img{display:block; max-width:none; max-height:none; image-rendering:auto}

    .pdf-wrap{position:absolute; inset:0; background:#111827}
    .pdf-wrap iframe{width:100%;height:100%;border:0;background:#111827}
    .pdf-fallback{
      position:absolute; inset:0; display:none; align-items:center; justify-content:center;
      gap:12px; flex-direction:column; text-align:center; padding:20px; background:#111827;
    }

    footer{padding:10px 14px;background:var(--panel);border-top:1px solid var(--line);display:flex;gap:10px;align-items:center}
...
<body>
  ...
  (function(){
    ...
    let scale = 1, deg = 0, panX = 0, panY = 0;
    let naturalW = 0, naturalH = 0;
    let isPanning = false, startX = 0, startY = 0;
    const MIN=0.1, MAX=8.0, STEP=0.1;

    img?.addEventListener('load', () => { naturalW = img.naturalWidth; naturalH = img.naturalHeight; fitToScreen(); updateStatus(); });
    ...
    function applyTransform(){ canvas.style.transform = `translate(-50%,-50%) translate(${panX}px,${panY}px) scale(${scale}) rotate(${deg}deg)`; }
    function fitToScreen(){ scale = getFitScale(); panX = 0; panY = 0; applyTransform(); }
    function set100(){ scale = 1; panX = 0; panY = 0; applyTransform(); }
    function resetView(){ deg = 0; fitToScreen(); }
    function clamp(v,a,b){ return Math.max(a, Math.min(b, v)); }
    function zoomAt(factor, x, y){
      const prev = scale; scale = clamp(scale * factor, MIN, MAX);
      const rect = stage.getBoundingClientRect();
      const cx = x - rect.left - rect.width/2, cy = y - rect.top - rect.height/2;
      panX -= cx * (scale/prev - 1); panY -= cy * (scale/prev - 1); applyTransform();
    }

    stage?.addEventListener('wheel', (e)=>{ e.preventDefault(); zoomAt(e.deltaY>0?1-STEP:1+STEP, e.clientX, e.clientY); updateStatus(); }, {passive:false});
    stage?.addEventListener('mousedown', (e)=>{ if (e.button!==0) return; isPanning=true; startX=e.clientX; startY=e.clientY; stage.style.cursor='grabbing'; });
    window.addEventListener('mousemove', (e)=>{ if (!isPanning) return; panX+=e.clientX-startX; panY+=e.clientY-startY; startX=e.clientX; startY=e.clientY; applyTransform(); });
    window.addEventListener('mouseup', ()=>{ isPanning=false; stage.style.cursor='default'; });
    stage?.addEventListener('dblclick', ()=>{ Math.abs(scale-1)<.01?fitToScreen():set100(); updateStatus(); });

    fitBtn?.addEventListener('click', ()=>{ fitToScreen(); updateStatus(); });
    hundredBtn?.addEventListener('click', ()=>{ set100(); updateStatus(); });
    zoomInBtn?.addEventListener('click', ()=>{ zoomAt(1+STEP, stage.clientWidth/2, stage.clientHeight/2); updateStatus(); });
    zoomOutBtn?.addEventListener('click', ()=>{ zoomAt(1-STEP, stage.clientWidth/2, stage.clientHeight/2); updateStatus(); });
    rotateBtn?.addEventListener('click', ()=>{ deg=(deg+90)%360; fitToScreen(); updateStatus(); });
    resetBtn?.addEventListener('click', ()=>{ resetView(); updateStatus(); });
    window.addEventListener('resize', ()=>{ const fit=getFitScale(); if (Math.abs(scale-fit)<.02) fitToScreen(); });

    wireCommon();

    function updateStatus(){ const pct = Math.round(scale*100); status.textContent = `Zoom: ${pct}% • Rotate: ${deg}°`; }
    ...
    function setupPdf(){
      ...
      setTimeout(function () {
        if (!loaded) { if (fb) fb.style.display = 'flex'; }
      }, 800);
    }
  })();
  </script>
