<?php defined('BASEPATH') OR exit('No direct script access allowed'); $page_title = 'Edit Report'; ?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    :root{ --ink:#1e3a8a; --brand:#2563eb; --muted:#64748b; --line:#d9dee7; --bg:#f6f8fc; --bg2:#eef2f7; --chip:#eef2ff; --radius:14px; --r-sm:10px; --shadow:0 8px 22px rgba(2,6,23,.10); }
    body{background:linear-gradient(180deg,var(--bg),var(--bg2) 70%,#e9edf3)}
    .app{max-width:920px;margin:0 auto;padding:0 14px}
    ...
    .page-head .title{margin:0;font:700 24px/1.2 Poppins,system-ui;color:var(--ink)}
    ...
    .card-head h6{margin:0;font:800 13px/1 Poppins;color:var(--ink);letter-spacing:.3px}
    .card-body{padding:18px 16px}
    .help{font-size:12px;color:var(--muted)}
    label{font-weight:600}
    .btn-brand{background:#f5f8ff;border:1px solid var(--brand);color:#1e3a8a;border-radius:12px;font-weight:700;padding:.5rem .95rem}
    .btn-brand:hover{background:#e9f0ff}
    .btn-ghost{border:1px solid var(--line);border-radius:12px}
    .chip-tag{display:inline-flex;gap:6px;align-items:center;padding:6px 10px;border-radius:999px;background:var(--chip);border:1px solid #dbe3ff;color:#19328a;font-size:12px}
    .uploader{border:1px dashed #c9d3ea;border-radius:12px;padding:14px;background:#fbfcff}
    .uploader input[type=file]{display:block}
    .file-list{margin:8px 0 0;padding-left:18px}
    .file-list li{font-size:12.5px;color:#374151}
    .page-spacer{height:28px}
    .existing-files{margin-top:8px}
    .existing-files li{font-size:12.5px}
  </style>
</head>
<body>
  ...
  (function(){
    ...
    input.addEventListener('change', function(){
      ...
      Array.from(this.files).forEach(function(f){
        var li = document.createElement('li'); li.textContent = f.name; out.appendChild(li);
      });
    });
  })();
</script>
