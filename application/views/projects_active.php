...
<head>
  ...
  <style>
    ...
    .btn{display:inline-flex;align-items:center;gap:.5rem;padding:.5rem .9rem;border-radius:10px;font-weight:600;font-size:.9rem;transition:all .18s}
    .btn i{font-size:1.05rem}
    .btn-brand{background:var(--indigo-500);border:1px solid var(--indigo-500);color:#fff}
    .btn-brand:hover{background:var(--indigo-400);border-color:var(--indigo-400)}
    .btn-light{background:#fff;border:1px solid var(--line);color:var(--ink)}
    .btn-light:hover{background:#f1f5f9}
    .btn-danger-soft{background:#fff;border:1px solid #fecaca;color:#b91c1c}
    .btn-danger-soft:hover{background:#fee2e2}

    .form-control, select.form-control{
      width:100%;background:#fff;border:1px solid #111827;border-radius:10px;
      padding:.70rem .9rem;font-size:1rem;transition:border-color .18s ease, box-shadow .18s ease
    }
    ...
    .card-flat{background:var(--card);border:1px solid var(--indigo-200);border-radius:14px;
      box-shadow:var(--shadow-1);transition:transform .16s,box-shadow .18s,border-color .18s}
    .card-flat:hover{transform:translateY(-2px);box-shadow:var(--shadow-2);border-color:var(--indigo-400)}
    ...
    .p-cover{position:relative;height:160px;background:#f8fafc;display:flex;align-items:center;justify-content:center;
      box-shadow:inset 0 -12px 22px rgba(2,6,23,.05);overflow:hidden}
    .p-cover.hasimg{background-size:cover;background-position:center;box-shadow:inset 0 -38px 54px rgba(2,6,23,.18)}
    .p-overlay{position:absolute;inset:0;display:flex;align-items:flex-end;justify-content:center;padding:14px;
      background:linear-gradient(to top, rgba(15,23,42,.55), rgba(15,23,42,.15) 45%, rgba(15,23,42,0));
      color:#fff;text-decoration:none;font-weight:700;gap:.5rem;opacity:0;transform:translateY(6px);transition:.18s}
    ...
    .badge-muted{border-color:#cbd5e1;background:#f1f5f9;color:#334155}
    .chips{display:flex;flex-wrap:wrap;gap:.4rem}
    .chip{display:inline-flex;align-items:center;gap:6px;padding:.28rem .6rem;border-radius:9999px;border:1px solid var(--line);background:#fff;
      font-size:.8rem;color:#334155;text-decoration:none;transition:all .18s;box-shadow:0 2px 6px rgba(2,6,23,.06)}
    .chip:hover{border-color:var(--indigo-300);color:#1f2937}
    .meta{color:#94a3b8;font-size:.85rem}

    .form-label .eyebrow-hint{font-weight:500;font-size:.78rem;color:var(--muted);margin-left:.35rem}
    .input-wrap{position:relative}
    .input-wrap .mdi{position:absolute;left:12px;top:50%;transform:translateY(-50%);opacity:.55}
    .input-wrap .form-control, .input-wrap select.form-control{padding-left:2.1rem}
    .helper-inline{display:flex;gap:.5rem;align-items:center;margin-top:.35rem}
    .helper-inline .link{font-weight:600;cursor:pointer}
    .helper-inline .link:hover{text-decoration:underline}
    #customCatWrap{display:none}

    .select2-container--bootstrap4 .select2-selection{
      border-radius:10px;border:1px solid #111827;min-height:44px;
      display:flex;align-items:center;padding:.2rem .5rem;
    }
    .select2-container--bootstrap4 .select2-selection__rendered{line-height:1.2}
    .select2-container--bootstrap4 .select2-selection__clear{margin-right:.25rem}
    .select2-container--bootstrap4 .select2-selection--single .select2-selection__arrow{height:44px}

    .select2-container--bootstrap4 .select2-selection--multiple{
      min-height:44px; height:44px; overflow:hidden; align-items:center;
    }
    .select2-container--bootstrap4 .select2-selection--multiple .select2-selection__choice{ display:none; }
    .select2-container--bootstrap4 .select2-selection--multiple .select2-search__field{ margin-top:0;  }

    .select2-container { width:100% !important; }
    .select2-container--open .select2-dropdown { z-index: 3000; }
    ...
    .modal-backdrop-lite{position:fixed;inset:0;background:rgba(2,6,23,.45);z-index:1050;display:none;padding:20px}
    .modal-card{max-width:520px;margin:5vh auto;background:#fff;border:1px solid var(--indigo-200);border-radius:14px;padding:16px;box-shadow:var(--shadow-2)}

    #catChips .chip { position:relative; padding-right:26px; }
    #catChips .chip .chip-x{
      position:absolute; right:6px; top:50%; transform:translateY(-50%);
      border:0; background:transparent; line-height:1; font-size:16px; cursor:pointer;
    }
...
.addr-col { position: relative; overflow: visible; }
...
.addr-dd .select2-results__option{ white-space: normal; }

.select2-container--bootstrap4 .select2-selection__rendered{
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width:100%;
}
.cat-col { position: relative; overflow: visible; }
...
.cat-dd .select2-results__option{ white-space:normal; }
...
.cat-dd.select2-dropdown{
  ...
  min-width:0 !important; max-width:none !important; z-index:3000;
}
.cat-dd .select2-results__options{ max-height:60vh !important; overflow-y:auto !important; }
.cat-dd .select2-results__option{ white-space:normal; }
.select2-container--bootstrap4 .select2-selection{
...
select.form-control { color: var(--muted); }

.form-control.has-value { color: var(--ink) !important; }

.select2-container--bootstrap4 .select2-selection__rendered { color: var(--muted); }
.select2-container--bootstrap4.select2-has-value .select2-selection__rendered { color: var(--ink) !important; }
      ...
      <div class="main-panel">
        <div class="content-wrapper pb-0">
          <div class="app">
            ...
            <?php
              if (!function_exists('is_img')) { function is_img($f){return in_array(strtolower(pathinfo($f,PATHINFO_EXTENSION)),['jpg','jpeg','png','webp']);} }
              if (!function_exists('is_pdf')) { function is_pdf($f){return strtolower(pathinfo($f,PATHINFO_EXTENSION))==='pdf';} }
              if (!function_exists('first_img')) { function first_img($arr){foreach(($arr??[]) as $f){ if(is_img($f)) return $f; } return null;} }
              if (!function_exists('first_file')) { function first_file($arr){foreach(($arr??[]) as $f){ return $f; } return null;} }
...
(function(){
  ...
  jQuery(document).on('click', '#catChips .chip .chip-x', function(){
    ...
    var $opt = $sel.find('option').filter(function(){ return jQuery(this).val() == val; });
    if ($opt.length === 0) { $opt = jQuery('<option>').val(val).text(val).appendTo($sel); }
...
(function(){
  ...
  $prov.select2({...s2, placeholder:'Select province',          dropdownParent:$provParent});
  $city.select2({...s2, placeholder:'Select city/municipality', dropdownParent:$cityParent});
  $brgy.select2({...s2, placeholder:'Select barangay',          dropdownParent:$brgyParent});
  ...
  function safe(v){ return jQuery('<div>').text(v).html(); }
  ...
  function api(params){
    ...
    return fetch(url, {credentials:'same-origin'})
      .then(r => r.json())
      .then(j => { if(!j.ok) throw new Error(j.msg||'Address API error'); return j.items||[]; });
  }

  async function loadProvinces(){
    $prov.prop('disabled', true);
    try{
      const items = await api({scope:'province'});
      fill($prov, items);
  ...
  async function onProvince(init){
    const pv = $prov.val();
    fill($city, []); fill($brgy, []);
    $city.prop('disabled', !pv); $brgy.prop('disabled', true);
    if (!pv) return;

    try{
      const items = await api({scope:'city', province: pv});
      fill($city, items);
  ...
  async function onCity(init){
    const pv = $prov.val(), ct = $city.val();
    fill($brgy, []); $brgy.prop('disabled', !(pv && ct));
    if (!(pv && ct)) return;

    try{
      const items = await api({scope:'brgy', province: pv, city: ct});
      fill($brgy, items);
...
(function(){
  ...
  if (!opts.length) { payUnitSel.disabled = true; payUnitSel.classList.remove('has-value'); return; }
...
(function(){
  document.querySelectorAll('select.form-control.js-val').forEach(function(sel){
    function paint(){ sel.classList.toggle('has-value', !!sel.value); }
  ...
  if (window.jQuery && jQuery.fn.select2) {
    ...
    jQuery('select.form-control.js-val').each(function(){ markS2(jQuery(this)); });
    jQuery(document).on('change', 'select.form-control.js-val', function(){ markS2(jQuery(this)); });
