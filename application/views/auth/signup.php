...
.btn-close {
  ...
  width: 1rem; height: 1rem; border: 0; background: transparent;
  opacity: .7; cursor: pointer; line-height: 1;
}
.btn-close:hover { opacity: 1; }
.btn-close::before { content: "Ã—"; font-size: 1rem; }
.fade { transition: opacity .15s linear; opacity: 0; }
.fade.show { opacity: 1; }
.small { font-size: .875rem; }
.mb-3  { margin-bottom: 1rem; }
button[disabled] { pointer-events: none !important; user-select: none !important; outline: 0 !important; }
</style>
                    ...
                    <div class="hidden min-h-screen flex-col xl:flex">
                        ...
                        <div class="my-auto">
                            ...
                            <div class="mt-5 text-lg text-white opacity-70">
                                All your jobs and skilled workers—together in <br> one place with TrabaWHO?
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="my-10 flex py-5 xl:my-0 xl:py-0">
                        <div class="box relative p-5 before:absolute before:inset-0 before:mx-3 before:-mb-3 before:border before:border-foreground/10 before:bg-background/30 before:shadow-[0px_3px_5px_#0000000b] before:z-[-1] before:rounded-xl after:absolute after:inset-0 after:border after:border-foreground/10 after:bg-background after:shadow-[0px_3px_5px_#0000000b] after:rounded-xl after:z-[-1] after:backdrop-blur-md mx-auto my-auto w-full px-5 py-8 sm:w-3/4 sm:px-8 lg:w-2/4 xl:ml-24 xl:w-auto xl:p-0 xl:before:hidden xl:after:hidden">
                            ...
                            <div class="mt-2 text-center opacity-70 xl:hidden">
                                All your jobs and skilled workers—together in one place with TrabaWHO?
                            </div>
...
<style>
  ...
  Register
    ...
    <style>
      #pmModal{position:fixed;inset:0;display:none;align-items:flex-start;justify-content:center;padding:1rem;z-index:2147483647}
      #pmModal.show{display:flex}
      #pmBackdrop{position:absolute;inset:0;background:rgba(2,6,23,.62)}
      #pmPanel{position:relative;z-index:1;width:100%;max-width:900px;max-height:85vh;border-radius:14px;background:#fff;border:1px solid rgba(15,23,42,.08);box-shadow:0 24px 80px rgba(2,6,23,.35);overflow:hidden;display:flex;flex-direction:column}
      #pmHead{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid #e5e7eb}
      #pmTitle{font-weight:700}
      #pmBody{flex:1 1 auto;overflow:auto;max-height:calc(85vh - 110px);padding:16px 18px;line-height:1.6;color:#0f172a}
      #pmBody h4{margin:10px 0 6px;font-weight:800}
      #pmBody p,#pmBody li{font-size:.95rem}
      #pmFoot{position:sticky;bottom:0;background:#fff;border-top:1px solid #e5e7eb;padding:10px 18px;display:flex;justify-content:flex-end;gap:.5rem}
      .pm-btn{border-radius:10px;padding:.55rem 1rem;font-weight:700;border:1px solid #e5e7eb;background:#fff;cursor:pointer}
      .pm-btn-primary{background:#2563eb;border-color:#2563eb;color:#fff}
      .pm-btn:hover{filter:brightness(.97)}
      .pm-close:hover{opacity:.85;transform:scale(1.02)}
      .pm-btn:focus,.pm-close:focus{outline:0;box-shadow:0 0 0 3px rgba(43,77,165,.35)}
.alert-inline {
  display: flex;
  align-items: center;
  gap: .5rem;
    ...
    <div id="pmModal" role="dialog" aria-modal="true" aria-labelledby="pmTitle">
      ...
      <div id="pmPanel">
        ...
        <div id="pmBody">
          ...
          <p>Welcome to <strong>Trabawho</strong>. We connect skilled workers with individuals or employers who need their services. This Privacy Policy explains what data we collect, how we use it, and the choices you have.</p>
          ...
          <h4>3) Legal Bases</h4>
          <p>We process your data to perform our contract with you (providing the Trabawho service), for our legitimate interests (platform safety and service improvement), and to comply with legal obligations.</p>
          ...
          <h4>8) Children</h4>
          <p>Trabawho is not directed to children under 16 and we do not knowingly collect personal data from children.</p>
...
(function () {
  ...
  async function check() {
    ...
    if (!v) {
      setHint('', 'info'); setInputState('neutral');
      if (btn) btn.disabled = !(tos && tos.checked);
      return;
    }
    if (!isLikelyEmail(v)) {
      setHint('Enter a valid email.', 'err'); setInputState('err');
      if (btn) btn.disabled = true;
      return;
    }

    try {
      const res = await fetch(url + '?email=' + encodeURIComponent(v), { credentials: 'same-origin' });
      const data = await res.json();
      if (!data.ok) {
        setHint(data.msg || 'Unable to verify email.', 'err'); setInputState('err');
        if (btn) btn.disabled = true;
        return;
      }

      if (data.available) {
        setHint('Great, this email is available.', 'ok'); setInputState('ok');
        if (btn) btn.disabled = !(tos && tos.checked);
      } else {
        setHint('This email is already registered. Please log in or use another email.', 'err'); setInputState('err');
        if (btn) btn.disabled = true;
      }
    } catch(e) {
      setHint('Unable to verify email right now.', 'err'); setInputState('err');
      if (btn) btn.disabled = true;
    }
  }

  email.addEventListener('input', () => { clearTimeout(t); t = setTimeout(check, 350); });
...
(function () {
  ...
  function openModal(e){ if (e) e.preventDefault(); modal.classList.add('show'); document.body.style.overflow = 'hidden'; }
...
(function(){
  ...
  let greOk = false;
  window.recaptchaOk       = function(){ greOk = true;  syncBtn(); };
  window.recaptchaExpired  = function(){ greOk = false; syncBtn(); };
  window.recaptchaError    = function(){ greOk = false; syncBtn(); };
