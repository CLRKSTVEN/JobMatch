<?php defined('BASEPATH') OR exit('No direct script access allowed'); $page_title = 'Complaint #'.(int)$item->id; ?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    :root{
      --ink:#0f172a; --muted:#64748b; --line:#e5e7eb; --chip:#f8fafc;
      --surface:#ffffff; --surface-2:#fbfcff; --radius:14px; --shadow:0 8px 22px rgba(2,6,23,.08);
      --brand:#2563eb; --brand-600:#1d4ed8;
    }
    body{ font-family:"Poppins",system-ui,-apple-system,"Segoe UI",Roboto,Arial; color:var(--ink); }
    .cardx{ background:var(--surface); border:1px solid var(--line); border-radius:var(--radius); box-shadow:var(--shadow); }
    .cardx--pad{ padding:16px; }
    .subtle{ color:var(--muted); }
    .complaint-head{ display:flex; align-items:center; gap:12px; flex-wrap:wrap; margin-bottom:12px; }
    .pill{ display:inline-flex; align-items:center; gap:8px; padding:.4rem .7rem; border-radius:9999px; border:1px solid; font-weight:700; font-size:12px; }
    .dot{ width:8px; height:8px; border-radius:50%; }
    .pill--open{ background:#fff7ed; border-color:#fdba74; color:#9a3412; } .pill--open .dot{ background:#f59e0b; }
    .pill--under{ background:#eff6ff; border-color:#93c5fd; color:#1e3a8a; } .pill--under .dot{ background:#1e3a8a; }
    .pill--resolved{ background:rgba(251,191,36,.18); border-color:rgba(251,191,36,.48); color:#92400e; } .pill--resolved .dot{ background:#fbbf24; }
    .pill--dismissed{ background:#f1f5f9; border-color:#cbd5e1; color:#334155; } .pill--dismissed .dot{ background:#94a3b8; }
    .btn-lite{ display:inline-flex; align-items:center; gap:8px; background:#fff; border:1px solid var(--line); color:#111827; border-radius:10px; padding:.5rem .8rem; font-weight:700; text-decoration:none; }
    .btn-primary{ display:inline-flex; align-items:center; gap:8px; background:var(--brand); border:1px solid var(--brand-600); color:#fff; border-radius:10px; padding:.55rem 1rem; font-weight:700; text-decoration:none; }
    .grid-wrap{ display:grid; grid-template-columns: 2.1fr 1fr; gap:16px; }
    @media (max-width:1100px){ .grid-wrap{ grid-template-columns:1.6fr 1fr; } }
    ...
    .krow{ background:var(--surface-2); border:1px solid var(--line); border-radius:12px; padding:10px 12px; }
    .krow .k{ font-size:11px; text-transform:uppercase; letter-spacing:.35px; color:#64748b; font-weight:800; }
    .krow .v{ margin-top:4px; font-weight:600; }
    .body-text{ line-height:1.65; }
    .badge-chip{ display:inline-flex; align-items:center; gap:6px; border-radius:999px; padding:.35rem .6rem; font-weight:700; font-size:11.5px; letter-spacing:.25px; background:#ffe5e8; color:#9a0820; border:1px solid #ffc4cb; }
    .evidence-grid{ display:grid; grid-template-columns: repeat(auto-fill, minmax(180px,1fr)); gap:10px; }
    .evi{ position:relative; background:#f8fafc; border:1px solid var(--line); border-radius:12px; padding:10px; min-height:110px; display:flex; align-items:center; justify-content:center; text-align:center; }
    .evi a{ text-decoration:none; color:#1e3a8a; font-weight:700; word-break:break-word; }
    .file-pill{ position:absolute; top:8px; left:8px; font-size:10.5px; padding:.15rem .45rem; border-radius:9999px; background:#fff; border:1px solid var(--line); }
    .formx .form-group{ margin-bottom:12px; }
    .formx label{ font-size:12px; font-weight:700; color:#334155; }
    .formx .form-control{ border-radius:10px; }
    .stack-actions{ display:flex; gap:8px; flex-wrap:wrap; }
    @media (max-width:700px){ .stack-actions .btn-primary, .stack-actions .btn-lite{ width:100%; justify-content:center; } }
  </style>
</head>
<body>
