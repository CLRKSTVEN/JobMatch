...
<style>
    ...
    --radius:12px; --shadow-1:0 6px 16px rgba(2,6,23,.08);
    }
    ...
    .panel{background:#fff;border:1px solid var(--silver-300);border-radius:var(--radius);box-shadow:var(--shadow-1)}
    .panel-head{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--silver-200)}
    ...
    .composer-input{flex:1;border-radius:9999px;border:1px solid var(--silver-300);background:#f5f7fb;padding:.65rem 1rem;cursor:pointer}
    .composer-actions{display:flex;gap:.5rem;flex-wrap:wrap}
    .chip{display:inline-flex;align-items:center;gap:6px;padding:.35rem .6rem;border-radius:9999px;border:1px solid var(--silver-300);background:#fff;font-weight:700;font-size:12px}
    .chip .mdi{color:var(--blue-600)}
.chip {
    cursor: pointer;
}
    /* Post card */
    .post{border:1px solid var(--silver-300);border-radius:var(--radius);box-shadow:var(--shadow-1);background:#fff;margin-top:12px}
    .post .head{display:flex;justify-content:space-between;gap:10px;padding:12px;border-bottom:1px solid var(--silver-200)}
    .post .who{display:flex;gap:10px}
    .post .who .avatar{width:44px;height:44px;border-radius:50%;object-fit:cover}
    .post .name{font-weight:800;color:var(--blue-900);line-height:1}
    .post .meta{font-size:12px;color:#64748b}
    .post .body{padding:12px}
    .post .img{border-radius:10px;overflow:hidden}
    .post .img img{width:100%;height:auto;display:block}
    .post .foot{padding:8px 12px;border-top:1px solid var(--silver-200);display:flex;gap:6px}
    .btn-pill{border-radius:9999px}
    .btn-brand{border:1px solid var(--blue-600);background:#f5f8ff;color:#1e3a8a}
    .btn-brand:hover{background:var(--gold-600);border-color:var(--gold-700);color:#111}
    .toast-container{position:fixed;top:16px;right:16px;z-index:1060}

    /* Blue inputs */
    .brand-input, .form-control.brand-input { border-color: var(--blue-600) !important; }
    .brand-input:focus { box-shadow: 0 0 0 .2rem rgba(37,99,235,.15); border-color: var(--blue-600); }
...
border-radius:18px;border:1px solid #c7d2fe; /* indigo-200 */
box-shadow:0 18px 44px rgba(2,6,23,.12),0 6px 18px rgba(2,6,23,.08);
overflow:hidden;background:#fff;
}
.tw-modal .modal-header{
padding:18px 22px;border-bottom:1px solid #c7d2fe;
...
.tw-modal .tw-card + .tw-card{margin-top:12px}
.tw-modal .tw-card h6{
font-weight:800;font-size:1rem;margin:0 0 10px 0;
display:flex;align-items:center;gap:8px
}
.tw-modal .tw-muted{color:#6b7280}
.tw-modal .tw-help{font-size:.85rem;color:#6b7280}

.tw-modal .tw-seg{
display:flex;background:#f4f6ff;border:1px solid #c7d2fe;border-radius:12px;overflow:hidden
}
.tw-modal .tw-seg label{
flex:1;padding:10px 12px;margin:0;border-right:1px solid #c7d2fe;
cursor:pointer;font-weight:600;text-align:center
}
.tw-modal .tw-seg label:last-child{border-right:0}
...
.tw-modal .tw-preview{display:flex;flex-wrap:wrap;gap:10px}
.tw-modal .proj-thumb{width:76px;height:76px;border-radius:10px;border:1px solid #e5e7eb;object-fit:cover}
.tw-modal .proj-chip{display:inline-flex;align-items:center;gap:6px;padding:.28rem .6rem;border-radius:9999px;
border:1px solid #e5e7eb;background:#fff;font-size:.82rem;text-decoration:none;box-shadow:0 2px 6px rgba(2,6,23,.06)}
.tw-modal .proj-chip i{font-size:1rem}

/* Switches (invitation + disclaimer) */
.form-switch.tw-switch{padding-left:0;display:flex;align-items:center;gap:.5rem}
.form-switch.tw-switch .form-check-input{
margin-left:0;margin-right:.6rem;width:3.1rem;height:1.6rem;background-color:#e5e7eb;border:1px solid #cbd5e1;background-image:none;border-radius:9999px
}
.form-switch.tw-switch .form-check-input:checked{background-color:#6366f1;border-color:#6366f1}
.form-switch.tw-switch .form-check-label{margin:0;line-height:1.4}

/* Disclaimer box */
.tw-modal .tw-disclaimer{border:1px dashed #a5b4fc;background:#fbfbff;border-radius:14px;padding:14px}
.tw-modal .tw-disc-head{display:flex;align-items:center;gap:10px;font-weight:800}
...
#hireSendBtn.disabled, #hireSendBtn:disabled{pointer-events:none;opacity:.6}
...
.btn-scope {
  ...
  padding: .35rem .9rem;
  font-weight: 700;
  border: 2px solid #c7d2fe;       /* indigo-200 outline */
  background: #fff;
...
(function(){
  ...
  if (posted && window.bootstrap) {
    new bootstrap.Toast(document.getElementById('postToast'), {delay: 2500}).show();
  }
})();
...
(function () {
  ...
  function absUrl(p){ return isHttp(p) ? p : (p ? BASE_URL + p.replace(/^\/+/, '') : ""); }
  ...
  function prependBatchDESC(items){
    ...
    for (var i = items.length - 1; i >= 0; i--){
      ...
      var tmp = document.createElement('div'); tmp.innerHTML = renderPostCard(it);
  ...
  function poll(){
    ...
    (function loop(){
      ...
      fetch(url, { credentials:'same-origin', headers:{'Accept':'application/json'}, cache:'no-store' })
        ...
        .finally(function(){ setTimeout(loop, POLL_MS); });
    })();
  }
