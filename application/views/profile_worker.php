...
<head>
  ...
  <title><?= htmlspecialchars($page_title ?? 'Worker Profile', ENT_QUOTES, 'UTF-8') ?> • Trabawho</title>
...
<link rel="stylesheet" href="<?= base_url('assets/css/mobile-patch.css?v=20251006') ?>">
  ...
  <style>
    :root{
      ...
      --brand:#6366f1; --brand-200:#c7d2fe; --brand-50:#eef2ff; --accent:#22c55e; --warn:#f59e0b;
      --shadow-1:0 6px 18px rgba(2,6,23,.08); --shadow-2:0 16px 38px rgba(2,6,23,.14);
    }
    body{background:var(--bg);color:var(--ink);font-family:"Poppins",ui-sans-serif,system-ui,-apple-system}
    .content-wrapper{padding-top:.8rem}
    ...
    .profile-card{position:relative;border-radius:16px;overflow:hidden;background:var(--card);box-shadow:var(--shadow-1);border:1px solid var(--line)}
    .profile-cover{height:180px;background:url('<?= base_url("assets/images/trabawhotext1.jpg") ?>') center/cover no-repeat}
    .profile-main{display:grid;grid-template-columns:96px 1fr auto;gap:18px;align-items:center;padding:18px}
    .avatar{width:96px;height:96px;border-radius:50%;object-fit:cover;border:4px solid var(--card);margin-top:-64px;box-shadow:0 10px 24px rgba(2,6,23,.18)}
    .profile-title{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
    .profile-name{font-size:clamp(18px,1.9vw,22px);font-weight:700;margin:0}
    .profile-sub{color:var(--muted);font-weight:600;margin-top:2px}
    .meta{color:var(--muted);font-size:.95rem}
    .quick-actions{display:flex;gap:10px;flex-wrap:wrap}
    .btn-soft{display:inline-flex;align-items:center;gap:8px;padding:.55rem .85rem;border-radius:12px;border:1px solid var(--brand-200);background:var(--brand-50);font-weight:700;color:#3730a3;text-decoration:none;transition:all .18s ease}
    .btn-soft:hover{transform:translateY(-1px);box-shadow:var(--shadow-1)}

    .stat-row{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:0 18px 18px}
    @media (max-width:768px){.profile-main{grid-template-columns:72px 1fr}.avatar{width:72px;height:72px;margin-top:-48px}.stat-row{grid-template-columns:repeat(2,1fr)}}
    .stat{background:#fff;border:1px solid var(--line);border-radius:12px;padding:.65rem .8rem;display:flex;align-items:center;gap:10px}
    .stat i{font-size:20px;color:#94a3b8}
    .stat .v{font-weight:800}

    .box-grid{margin-top:18px;display:grid;grid-template-columns:repeat(12,1fr);gap:16px}
    .box{grid-column:span 6;background:var(--card);border:1px solid var(--line);border-radius:14px;box-shadow:var(--shadow-1);padding:16px}
    .box.wide{grid-column:span 12}
    .box-head{display:flex;align-items:center;gap:10px;margin-bottom:10px}
    .box-head i{font-size:22px;color:#94a3b8}
    .box-head h6{margin:0;font-size:15px;font-weight:800;letter-spacing:.2px}
    .box-body{color:var(--ink)}
    .empty{color:var(--muted);border:1px dashed var(--line);border-radius:12px;padding:14px;text-align:center}

    .pill{display:inline-flex;align-items:center;gap:6px;padding:.28rem .6rem;border-radius:9999px;border:1px solid var(--brand);background:var(--brand);font-size:.85rem;font-weight:700;color:#fff;text-decoration:none;transition:.18s}
    .pill i{color:#fff}
    a.pill:hover{background:#1d4ed8;border-color:#1d4ed8}
    .chips{display:flex;flex-wrap:wrap;gap:.45rem}
    .chip{display:inline-flex;align-items:center;gap:.4rem;padding:.24rem .55rem;border-radius:9999px;border:1px solid var(--line);background:#fff;font-size:.8rem}

    .c-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px}
    .c-card{position:relative;height:120px;border:1px solid var(--line);border-radius:12px;overflow:hidden;background:#f8fafc;display:flex;align-items:center;justify-content:center;transition:.16s ease}
    .c-card:hover{transform:translateY(-3px);box-shadow:var(--shadow-2)}
    .c-card.hasimg{background-size:cover;background-position:center;box-shadow:inset 0 -34px 60px rgba(2,6,23,.22)}
    .c-overlay{position:absolute;inset:0;display:flex;align-items:flex-end;justify-content:center;padding:10px;background:linear-gradient(to top, rgba(15,23,42,.60), rgba(15,23,42,.15) 50%, rgba(15,23,42,0));opacity:0;transition:.18s}
    .c-card:hover .c-overlay{opacity:1}
    .c-tag{display:inline-flex;align-items:center;gap:.45rem;padding:.35rem .6rem;border-radius:9999px;background:rgba(255,255,255,.16);color:#fff;font-weight:800}
    .c-icon{font-size:38px;color:#cbd5e1}
  ...
  #hireModal .proj-thumb {
    ...
    border:1px solid #e5e7eb; flex:0 0 auto; box-shadow:0 2px 6px rgba(2,6,23,.06);
    max-width:none; /* override Bootstrap img {max-width:100%} */
  }
  #hireModal .proj-chip {
    display:inline-flex; align-items:center; gap:6px; padding:.28rem .6rem;
    border-radius:9999px; border:1px solid #e5e7eb; background:#fff; font-size:.82rem;
    text-decoration:none; box-shadow:0 2px 6px rgba(2,6,23,.06);
  }
  #hireModal .proj-chip i { font-size:1rem; }
  .c-cap{
  margin-top:6px;font-size:.82rem;line-height:1.2;
...
@media (max-width: 768px){
  ...
  .profile-cover{ height:110px; background-size:cover !important; }
  .profile-main{
    grid-template-columns:64px 1fr; gap:10px 12px; padding:12px;
  }
  .avatar{ width:64px; height:64px; margin-top:-44px; border-width:3px; }
  .profile-title{ gap:8px; }
  .profile-name{ font-size:18px; }
  .profile-sub,.meta{ font-size:12px; }
  .quick-actions{ width:100%; }
  .quick-actions .btn-soft{ flex:1 1 160px; justify-content:center; }
  .stat-row{ grid-template-columns:1fr 1fr; gap:8px; padding:0 12px 12px; }
  .stat{ padding:.6rem .7rem; }
  .box-grid{ grid-template-columns:1fr; gap:12px; margin-top:12px; }
  .box,.box.wide{ grid-column:1 / -1; padding:14px; border-radius:16px; }
  .box-head h6{ font-size:14px; }
  .c-card{ height:120px; }
  .c-overlay{ padding:10px; }
  .c-tag{ font-size:11.5px; padding:.35rem .65rem; }
  .box .table-responsive{ overflow:visible; }
  .box .table thead{ display:none; }
  .box .table{ border-collapse:separate; border-spacing:0 12px; }
  .box .table tbody, .box .table tr, .box .table td{ display:block; width:100%; }
  .box .table tbody tr{
    background:#fff; border:1px solid var(--line);
    border-radius:14px; box-shadow:var(--shadow-1); padding:10px 12px;
  }
  .box .table tbody tr td{
    display:grid; grid-template-columns:minmax(90px,34%) 1fr;
    gap:8px; align-items:baseline; padding:8px 0; border-bottom:1px dashed #e5e7eb;
  }
  .box .table tbody tr td:last-child{ border-bottom:0; padding-bottom:2px; }
  .box .table td::before{
    content:attr(data-th); text-transform:uppercase; letter-spacing:.32px;
    font:700 10px/1 Poppins,system-ui,-apple-system,"Segoe UI",Roboto; color:#64748b;
  }
  .box .table td > *{ overflow-wrap:anywhere; word-break:break-word; }
  .chip{ font-size:.78rem; padding:.22rem .5rem; }
  #svcMixPanel .box-body > div{ grid-template-columns:1fr !important; }
  #svcMixLegend{ margin-top:10px; }
}
...
if (empty($certs) && isset($w)) {
  ...
  function wp_path_of($f){ return is_array($f) ? (string)($f['path'] ?? '') : (string)$f; }
}
if (!function_exists('wp_name_of')) {
  function wp_name_of($f){ return trim(is_array($f) ? (string)($f['name'] ?? '') : ''); }
}
if (!function_exists('wp_is_img')) {
  function wp_is_img($f){ $p = wp_path_of($f); return in_array(strtolower(pathinfo($p, PATHINFO_EXTENSION)), ['jpg','jpeg','png','webp']); }
}
...
<img
                ...
                <div>
                  ...
                  <?php if ($loc || $phone) { ?>
                    <div class="meta mt-1">
                      ...
                      <?php if ($phone) { $tel = preg_replace('/[^\d+]/', '', $phone); ?>
  ...
  foreach (['user_id','id','workerID','worker_id','uid'] as $k) {
    if (isset($w->$k) && is_numeric($w->$k) && (int)$w->$k > 0) { $recipient_id = (int)$w->$k; break; }
  }

  $myId = 0;
  foreach (['id','user_id','uid','account_id'] as $k) {
    $v = $this->session->userdata($k);
    if (is_numeric($v)) { $myId = (int)$v; break; }
  }
...
<div class="box-head"><i class="mdi mdi-file-certificate-outline"></i><h6>Clearances &amp; Certificates</h6></div>
                <div class="box-body">
                  <?php if (empty($certs)) { ?>
                ...
                <?php
  $imgCerts = []; $docCerts = [];
  foreach ($certs as $f) { if (wp_is_img($f)) $imgCerts[] = $f; else $docCerts[] = $f; }
?>
<?php if (!empty($imgCerts)) { ?>
  <div class="c-grid mb-2">
    <?php foreach ($imgCerts as $f) {
      ...
      if ($basePath !== '' && strpos($fParam, $basePath.'/') === 0) { $fParam = substr($fParam, strlen($basePath) + 1); }
...
<?php } ?>
              ...
              <section class="box">
                <div class="box-head"><i class="mdi mdi-lightbulb-on-outline"></i><h6>Skills & Rates</h6></div>
                <div class="box-body">
              ...
              <section class="box">
                <div class="box-head"><i class="mdi mdi-information-outline"></i><h6>About</h6></div>
                <div class="box-body">
                  <?php if (!empty($bio)) { ?>
...
?>
<section class="box wide">
  <div class="box-head"><i class="mdi mdi-briefcase-outline"></i><h6>Work Experience</h6></div>
  <div class="box-body">
    <?php if (empty($xp)) { ?>
    ...
    <?php } ?>
              ...
              <section class="box wide">
                <div class="box-head"><i class="mdi mdi-briefcase-outline"></i><h6>Portfolio</h6></div>
                <div class="box-body">
                  <?php if (empty($items)) { ?>
                        ...
                        <tbody>
                          <?php foreach ($items as $it) { 
                            ...
                            if (!empty($it['created_at'])) {
                              ...
                              if ($ts !== false) { $dateStr = date('M d, Y', $ts); }
                            }
                          ?>
                            <tr>
                              <td>
...
<section class="box wide">
  <div class="box-head">
    <i class="mdi mdi-comment-text-outline"></i><h6>Latest Reviews</h6>
  </div>
  <div class="box-body">
...
(function () {
  ...
  function renderPreview(meta){
    ...
    if (!meta || !Array.isArray(meta.files) || !meta.files.length) {
      ...
      imgs.forEach(f => { html += '<img class="proj-thumb" src="'+esc(f.url)+'" alt="'+esc(f.name)+'">'; });
      pdfs.forEach(f => { html += '<a class="proj-chip" href="'+esc(f.url)+'" target="_blank" rel="noopener"><i class="mdi mdi-file-pdf-box"></i> '+esc(f.name)+'</a>'; });
      html += '</div>';
  ...
  function fetchAndPreview(pid){
    if (!pid) { H.preview && (H.preview.innerHTML=''); applyProjectBillingToModal(null); return; }
    fetch('<?= site_url("projects/api/one/") ?>'+pid, { credentials:'same-origin' })
      .then(r => r.json()).then(res => { if (!res || !res.ok) throw 0; renderPreview(res); })
      .catch(() => { H.preview && (H.preview.innerHTML='<div class="text-muted small">Could not load files.</div>'); applyProjectBillingToModal(null); });
  }
  ...
  function loadProjectsIntoModal(){
    ...
    fetch('<?= site_url("projects/api/active-min") ?>', { credentials: 'same-origin' })
      ...
      .then(res => {
        ...
        items.forEach(p => { html += '<option value="'+p.id+'">'+esc(p.title)+'</option>'; });
  ...
  document.querySelectorAll('.hire-btn').forEach(function(btn){
    btn.addEventListener('click', function(e){
      ...
      H.sendBtn.disabled = false; H.sendBtn.classList.remove('disabled');
      H.modal.show();
    });
  });
  ...
  H.sendBtn.addEventListener('click', function(){
    ...
    .then(r => r.ok ? r.json() : { ok:false, message: 'HTTP '+r.status })
    .then(res => {
      var ok = !!(res && res.ok);
      if (ok) {
        H.modal.hide();
    ...
    .catch(() => { toast('Could not send hire request', 'error'); });
  });
  ...
  document.addEventListener('click', function (e) {
    ...
    const c = csrfPair(); if (c){ fd.append(c.name, c.hash); }
    fd.append('to', to);
...
(function(){
  ...
  function renderFiles(files){
    ...
    imgs.forEach(f => { html += '<img src="'+esc(f.url)+'" class="proj-thumb" style="width:84px;height:84px;object-fit:cover;border:1px solid #e5e7eb;border-radius:10px">'; });
    pdfs.forEach(f => { html += '<a href="'+esc(f.url)+'" target="_blank" rel="noopener" class="proj-chip"><i class="mdi mdi-file-pdf-box"></i> '+esc(f.name)+'</a>'; });
    html += '</div>';
    return html;
  }
  ...
  function render(){
    ...
    if (!STAT && INV && INV.can_act && role === 'worker') {
      ...
      function send(action){
        if (!tid || !INV?.pid) { window.showToast && window.showToast('Missing thread or project id', 'error'); return; }
        const fd = new FormData();
...
(async function(){
      ...
      options: {
        ...
        plugins:{
          ...
          tooltip:{ callbacks:{ label: (ctx) => {
            const c = ctx.parsed, tot = values.reduce((a,b)=>a+b,0) || 1;
  ...
  function makePill(key, label, total){
    ...
    btn.addEventListener('click', () => {
      current = key; setActivePill(); setCaption(current);
      const d0 = dataMap[current] || {rows:[],total:0};
      drawPie(d0.rows); legend(d0.rows, d0.total);
    });
    return btn;
  }
...
(function(){
  ...
  tables.forEach(t=>{
    ...
    if (tb) new MutationObserver(()=>labelize(t)).observe(tb,{childList:true,subtree:true});
  });
})();
</script>
