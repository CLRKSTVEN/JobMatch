...
<head>
  ...
  <title><?= html_escape($page_title ?? 'Edit Client Profile') ?> - Trabawho</title>
  ...
  <style>
    ...
    .status-badge{display:inline-flex;align-items:center;gap:.5rem;color:#fff;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);border-radius:9999px;padding:.4rem .75rem;line-height:1;box-shadow:0 1px 0 rgba(255,255,255,.12) inset,0 8px 24px rgba(0,0,0,.12);backdrop-filter:saturate(140%) blur(4px)}
    .upload-previews{display:grid;grid-template-columns:repeat(1,minmax(0,1fr));gap:1rem}
    @media (min-width:640px){.upload-previews{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media (min-width:1280px){.upload-previews{grid-template-columns:repeat(3,minmax(0,1fr))}}
    .doc-card-xl{aspect-ratio:5/4}
    .doc-pdf-pane{background:linear-gradient(180deg,#f8fafc 0%,#eef1f5 38%,#d2d6dc 60%,#9aa0a6 100%)}
    .doc-pdf-badge{font-size:11px;line-height:1;padding:.35rem .55rem;border-radius:.5rem;background:#b91c1c;color:#fff;font-weight:700;box-shadow:0 1px 0 rgba(0,0,0,.12) inset,0 6px 16px rgba(0,0,0,.15)}
    .doc-view-btn{position:absolute;left:50%;bottom:14px;transform:translateX(-50%);display:inline-flex;align-items:center;gap:.4rem;padding:.5rem .9rem;border-radius:9999px;background:#2563eb;color:#fff;font-weight:700;font-size:.85rem;box-shadow:0 8px 20px rgba(37,99,235,.35)}
    .doc-view-btn:hover{filter:brightness(1.05);transform:translateX(-50%) translateY(-1px)}
...
<?php
  ...
  $normalize_rel = function($rel){ $rel = str_replace('\\','/', (string)$rel); return ltrim($rel, '/'); };
  ...
  if ($avatarField !== '' && preg_match('#^https?://#i', $avatarField)) {
    ...
    if (!is_file(FCPATH.$rel)) { $rel = is_file(FCPATH.$DEFAULT_AVATAR_REL) ? $DEFAULT_AVATAR_REL : null; }
...
<script>
  ...
  (function(){
    const inp=document.getElementById('avatar'); const img=document.getElementById('avatarPreview'); const wrap=document.getElementById('preview-avatar');
    if(!inp) return;
    inp.addEventListener('change', e=>{
      ...
      r.onload=ev=>{
        ...
        if(wrap){ wrap.innerHTML=''; const i=document.createElement('img'); i.src=ev.target.result; i.className='w-[140px] h-auto rounded-xl border border-gray-200'; wrap.appendChild(i); }
      };
      r.readAsDataURL(f);
    });
  })();
  ...
  (function(){
    ...
    function fill(el, items, ph){
      ...
      if ($sel && $sel.data && $sel.data('select2')) {
        ...
        items.forEach(v=>{const opt=document.createElement('option'); opt.value=v; opt.textContent=v; el.appendChild(opt);});
      }
    }

    async function api(params){
      const url=apiBase+'?'+new URLSearchParams(params).toString();
      const res=await fetch(url,{credentials:'same-origin'}); const txt=await res.text();
      if(!res.ok) throw new Error('HTTP '+res.status+' '+txt.slice(0,120));
      const j=JSON.parse(txt); if(!j.ok) throw new Error(j.msg||'Address API error'); return j.items||[];
    }

    async function loadProvinces(){
      citySel.disabled=true; brgySel.disabled=true; fill(provinceSel, [], 'Loading provinces…');
      try{
        const items=await api({scope:'province'}); fill(provinceSel, items, '- Select Province -');
      }catch(e){ console.error(e); fill(provinceSel, [], 'Unavailable — refresh to retry'); return; }
    ...
    async function onProvince(init=false){
      const pv=provinceSel.value; citySel.disabled=true; brgySel.disabled=true;
      fill(citySel, [], '- Select City -'); fill(brgySel, [], '- Select Barangay -'); if(!pv) return;
      try{
        const items=await api({scope:'city', province:pv}); fill(citySel, items, '- Select City -');
      }catch(e){ console.error(e); fill(citySel, [], 'Unavailable — refresh to retry'); return; } finally{ citySel.disabled=false; }
      if(init && preCity && [...citySel.options].some(o=>o.value===preCity)){ citySel.value=preCity; await onCity(true); }
    }
    async function onCity(init=false){
      const pv=provinceSel.value, ct=citySel.value; brgySel.disabled=true; fill(brgySel, [], '- Select Barangay -'); if(!pv||!ct) return;
      try{
        const items=await api({scope:'brgy', province:pv, city:ct}); fill(brgySel, items, '- Select Barangay -');
      }catch(e){ console.error(e); fill(brgySel, [], 'Unavailable — refresh to retry'); return; } finally{ brgySel.disabled=false; }
      if(init && preBrgy && [...brgySel.options].some(o=>o.value===preBrgy)){ brgySel.value=preBrgy; }
    }
...
<script>
(function(){
  function toast(msg, ok=true){ (ok ? console.log : console.error)(msg); }
  ...
  if (certGrid) {
    certGrid.addEventListener('click', async (e) => {
      ...
      try {
        ...
        showFlash(data.flash || { type: 'success', text: 'Document deleted.' });
  ...
  document.addEventListener('click', async (e) => {
    ...
    try {
      ...
      showFlash(data.flash || { type: 'success', text: 'File deleted.' });
