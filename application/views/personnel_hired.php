...
<head>
  ...
  <title><?= htmlspecialchars($page_title ?? 'Hired Personnel', ENT_QUOTES, 'UTF-8') ?> • Trabawho</title>
  ...
  <style>
    ...
    body{background:var(--bg);color:var(--ink);font-family:"Poppins",ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial}
    .content-wrapper{padding-top:1rem}
    .app{max-width:1120px;margin:0 auto;padding:0 16px}

    .eyebrow{font-size:.85rem;color:var(--muted);font-weight:600;letter-spacing:.3px;margin-bottom:.2rem}
    h4{font-size:clamp(18px,1.9vw,22px);font-weight:800;margin:0}

    .section{background:#fff;border:1px solid var(--indigo-200);border-radius:14px;box-shadow:var(--shadow-1);padding:16px}
    .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px}
    .card-flat{background:#fff;border:1px solid var(--indigo-200);border-radius:14px;box-shadow:var(--shadow-1);transition:.18s}
    .card-flat:hover{border-color:var(--indigo-400);box-shadow:var(--shadow-2);transform:translateY(-2px)}
    .card-body{padding:14px}
    .card-footer{padding:12px 14px;border-top:1px solid var(--line);background:#fff}

    .avatar{width:52px;height:52px;border-radius:9999px;object-fit:cover;border:2px solid var(--line)}
    .title{font-weight:700}
    .muted{color:var(--muted)}

    .pill{display:inline-flex;align-items:center;gap:6px;padding:.36rem .7rem;border-radius:9999px;border:1px solid var(--indigo-200);
      background:#fff;font-size:.82rem;font-weight:600;color:#334155;text-decoration:none;transition:all .18s ease}
    .pill:hover{background:var(--indigo-500);border-color:var(--indigo-500);color:#fff}
    .badge-soft{display:inline-flex;align-items:center;border-radius:9999px;padding:.2rem .55rem;font-weight:600;font-size:.78rem;
